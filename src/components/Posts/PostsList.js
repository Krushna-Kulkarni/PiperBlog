...
export default function PostsList() {
  ...
  const {categoryList, loading:catLoading, appErr:catAppErr, serverErr:catServerErr} = category; //renamed 
            ...
            <div className="flex flex-wrap -mx-3">
              <div className="mb-8 lg:mb-0 w-full lg:w-1/4 px-3">
                <div className="py-4 px-6 bg-gray-600 shadow rounded">
                  ...
                  <ul>
                    {catLoading ? (<LoadingComponent />) : catAppErr || catServerErr ? (<h1>{catServerErr}{catAppErr}</h1>) : categoryList?.length <=0 ? (<h1 className="text-yellow-400 text-lg">No Category Found</h1>) : categoryList?.map((category) => (
                        <li>
              ...
              <div className="w-full lg:w-3/4 px-3">
                ...
                {appErr || serverErr ? (<h1 className="text-red-400 text-center text-lg">{serverErr} {appErr}</h1>): postLists?.length <=0 ? (<h1 className="text-yellow-400 text-lg text-center">No Post Found</h1>):postLists?.map((post) => ( <div key={post?.id} className="flex flex-wrap bg-gray-900 -mx-3  lg:mb-6">
