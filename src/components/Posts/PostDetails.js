...
const PostDetails = () => {
  ...
  }, [id,commentCreated,commentDeleted, dispatch])
    ...
    <>
      {loading ? (<div><LoadingComponent/></div>) : appErr || serverErr ? (<h1 className="h-screen text-red-600 text-xl">{serverErr}{appErr}</h1>):(<section className="py-20 2xl:py-40 bg-gray-800 overflow-hidden">
        <div className="container px-4 mx-auto">
          {/* Post Image */}
          <img
            className=" mb-24 m-auto w-auto h-auto object-cover"
            src={postDetails?.image}
            alt=""
          />
          <div className="max-w-2xl mx-auto text-center">
            <h2 className="mt-7 mb-14 text-5xl 2xl:text-5xl text-white font-bold font-heading">
              {postDetails?.title} 
            </h2>
